...
hse_raw <- read_dta("U:/Modelling/R Project/data_raw/hse_2022_eul_v1.dta")
...
count(hse_baseline_11_17_clean, imd_group, sort = TRUE)

# ---- 5) Structural placeholder: education status -------------------------------

# Education status is included as a structural characteristic for modelling.
# Under-16s are deterministically in compulsory education at model initialisation.
# For the 16–19 age band (which includes 16–17 at model initialisation), we set a
# placeholder that can be refined later using Understanding Society.

hse_baseline_11_17_clean <- hse_baseline_11_17_clean |>
  mutate(
    education_status = case_when(
      age35g %in% c("11-12", "13-15") ~ "in_compulsory_education",
      age35g == "16-19" ~ "post_16_education",   # placeholder for now
      TRUE ~ NA_character_
    ),
